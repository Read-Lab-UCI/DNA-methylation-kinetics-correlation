...
for loopset=1:3
    ...
    for repnum=1:3
        ...
        SimName=['SimRepliBSDist_' setname '_' num2str(repnum)];
        ...
        for j=1:numel(Times)
            ...
            for loopreads=1:NumReads
                ...
                %call the simulator
                [AllDat_each]=Distributive_Gillespie(sites,fracmeth,TimePts,SimName);
    ...
    sites=double(inputsites);
    sites=sites(:); %reset sites for each type of simulation from input file
    ...
    for repnum=1
        ...
        SimName=['SimWGBSDist_' setname '_' num2str(repnum)];
        datafilename=['AllDat_' SimName];
        for loopreads=1:NumReads
            
            TimePts=rand*30; %randomly selects a post-replication time for this read < 30 hours
            [AllDat_each]=Distributive_Gillespie(sites,fracmeth,TimePts,SimName);
